...
public class ChgSynClient extends Thread{
	...
	public ChgSynClient(ConfigReader cr, IndexableFs fs) throws IOException{
		...
		timeout = cr.getInt("timeout", timeout);
		this.serverDaemonPort = cr.getInt("serverDaemonPort");
		this.fs = fs;
	}
	public boolean canService(){
		return fs.isCanService();
	}
	public void run(){
		while(true){
			try {
	...
	public void query() throws Exception{
		...
		try{
			if(clt == null || !clt.socket.isOpen()){
				clt = new MessageClient(serverIp, serverDaemonPort, timeout);
			}
			...
			if(WatcherAction.isStatusOK(ans)){
				...
				log.debug("bad answer status! msg is %s", ans);
			}
