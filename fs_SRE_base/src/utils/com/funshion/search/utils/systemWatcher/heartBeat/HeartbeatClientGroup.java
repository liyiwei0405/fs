...
public class HeartbeatClientGroup {
	...
	private HBInfo getBeatConfig(ConfigReader cr) throws Exception{
		...
		if(ip == null){
			throw new Exception("ip error config for " + cr.sectionName + " of config file " + cr.configFile);
		}

		if(port == -1){
			throw new Exception("port error config for " + cr.sectionName + " of config file " + cr.configFile);
		}

		if(groupid < 0){
			throw new Exception("groupid error config for " + cr.sectionName + " of config file " + cr.configFile);
		}

		if(groupSize < 1){
			throw new Exception("groupSize error config for " + cr.sectionName + " of config file " + cr.configFile);
		}

		if(groupidx < 0 || groupidx >= groupSize){
			throw new Exception("groupidx error config for " + cr.sectionName + " of config file " + cr.configFile);
		}

		if(svcport <= -1){
			throw new Exception("svcport error config for " + cr.sectionName + " of config file " + cr.configFile);
		}
