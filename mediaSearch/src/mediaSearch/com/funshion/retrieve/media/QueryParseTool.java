...
public class QueryParseTool {
	...
	public static Query buildQuery(BooleanQuery bq, PortalFSCondition cond, Occur occ) throws TypeOperationCheckFailException, Exception{
		if(cond instanceof PortalRangeCondition){
			...
			AnaType aType = ITableDefine.instance.getAnaType(THIS.field.charAt(0));
			NumericRangeQuery<?>rq;
			if(aType == AnaType.aInt){
				rq = NumericRangeQuery.newIntRange(THIS.field, 
								Integer.parseInt(THIS.min), 
								Integer.parseInt(THIS.max),
								THIS.includeMin, THIS.includeMax);
			}else if(aType == AnaType.aFloat){
				rq = NumericRangeQuery.newFloatRange(THIS.field, 
								Float.parseFloat(THIS.min), 
								Float.parseFloat(THIS.max), 
								THIS.includeMin, THIS.includeMax);
			}else if(aType == AnaType.aFloat){
				rq = NumericRangeQuery.newLongRange(THIS.field, 
								Long.parseLong(THIS.min),
								Long.parseLong(THIS.max), 
								THIS.includeMin, THIS.includeMax);
			}else{
				throw new TypeOperationCheckFailException(aType + " is not rangable AnaType, for " + THIS);
			}
