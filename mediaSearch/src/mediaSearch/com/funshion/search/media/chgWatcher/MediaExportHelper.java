...
import com.funshion.search.utils.MongoHelper;
...
public class MediaExportHelper extends ExportChgHelper{
	...
	public void doExport(boolean totalExport, DatumFile dFile) throws Exception{
		...
		MongoHelper mongo = new MongoHelper(new ConfigReader(ConfUtils.getConfFile( "MediaExport/mongo.rdf"), "fs_video"));
		try{
			DBObject queryObject = new BasicDBObject("publishflag", "published").append("class", "normal");
			DBObject fieldObject = new BasicDBObject("videoid", true).append("types", true).append("video_media_ids", true).append("_id", false);


			DBCursor cur = mongo.col.find(queryObject, fieldObject).sort(new BasicDBObject("modifydate", -1));
	...
	private Map<Integer, List<Integer>> genRelatedVideoMap(DBCursor cur){
		log.warn("generating related video map..");
