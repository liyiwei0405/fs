...
class query_msg_module
{
	public static function get_service_info($hosts)
	{
		...
		{
			$daemon_message_rpc_service = new daemon_message_rpc_service($host['ip'],$host['port']);
			$query_msg_info = helper::obj_to_arr($daemon_message_rpc_service->get_query_msg_info());
			...
			foreach($query_msg_info['answerBody'] as $service)
				$service_result[] = self::_proc_service_infos(explode(',',$service));
		
			if($service_result)
				$service_results = array_merge_recursive($service_results,array($host['ip'] => $service_result));
	 ...
	 * @return array
	 */
	private static function _proc_service_infos($services){
		...
		foreach($services as $service)
		{
			...
			if(strstr($service,'RpcServerInstance')){
				...
				$rpc_service_methods[] = array(
						...
						'methods' => self::_proc_service_method($method_infos),
				);				
	...
	private static  function _proc_service_method($method_infos)
	{
		...
		foreach($method_infos as $method_info ){
			...
			if(strstr('NOT',$method_info)){
				...
				$head = !$data[0] ? array():self::_proc_method_head($data[0]);
				$body = !isset($data[1]) ? array():self::_proc_method_body($data[1]);
	 ...
	 */
	private static function _proc_method_body($string)
	{
	 ...
	 * 获取Location time等数据
	 */
	private static function _proc_method_head($string)
	{
		if(!is_string($string)) return;
		if(!$string) return;
